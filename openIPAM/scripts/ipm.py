...
class XMLRPCInterface(object):
    def __init__(self, username, password, url="https://127.0.0.1:8443/api/"):
        ...
        if url[:5] == "http:":
            ssl = False
        self.ipam = xmlrpc.client.ServerProxy(
            self.__url,
        ...
        while s <= e:
            ...
            while mask > 0:
                ...
                if bad_mask or bad_base:
                    # the mask is too big
                    next = (s | (bits >> 1)) + 1
                    net = openipam.iptypes.IP(
                        "%s/%s" % (str(openipam.iptypes.IP(s)), mask + 1)
                    )
                    s = next
                    nets.append(net)
                    break
...
class IPMCmdInterface(cmd.Cmd):
    ...
    def mkdict(self, line):
        ...
        if arg_len % 2:
            raise Exception(
                'command requires an even number of arguments, got "%s" which has %s'
                % (line, arg_len)
            )
        filter = {}
    ...
    def show_dns_dicts(self, dicts):
        ...
        fmt_str = (
            "\tname: %%(name)-%ds type: %%(type)-6s content: %%(content)-%ds prio: %%(priority)4s ttl: %%(ttl)5s view: %%(vid)5s id: %%(id)6s\n"
            % (name_l, content_l)
        )
        for record in dicts:
            r = record.copy()
    ...
    def do_show_addresses(self, arg):
        ...
        full_list = self.iface.get_addresses(
            network=filter["network"], order_by="addresses.address"
        )
        used_list = []
        free_list = []
    ...
    def do_show_mac(self, arg):
        ...
        if leases:
            ...
            self.show_dicts(
                leases,
                [("address", "address"), ("mac", "mac"), ("ends", "ends")],
                prefix="\t",
            )
        groups = []
        ...
        if groups:
            ...
            self.show_dicts(
                groups,
                [("name", "Group"), ("description", "Description"), ("id", "GID")],
                prefix="\t",
            )
        ...
        if owners:
            ...
            for owner in owners:
                ...
                if result:
                    ...
                    print(
                        "\tERROR: %s has ownership over host, but lookup (source: %s) failed!"
                        % (owner["username"], owner["source"])
                    )
    ...
    def do_show_network(self, arg=None):
        ...
        self.show_dicts(
            ...
            [
                ...
                ("network", "network"),
                ("gateway", "gateway"),
                ("shared_network", "Shared Network ID"),
                ("dhcp_group", "DHCP Group ID"),
            ],
            prefix="\t",
        )
        ...
        if self.get_bool_from_user("delete record(s)", default=False):
            for id in ids:
                try:
                    ...
                    failed.append(id)
            if failed:
                print("Failed to delete the following records: ")
            for i in failed:
                self.onecmd("show_dns id %s" % id)
    ...
    def do_add_domain(self, arg):
        ...
        self.iface.add_domain(
            name=name, typename=typename, description=desc, master=master
        )
    ...
    def do_add_external_domain(self, arg):
        ...
        self.iface.add_dns_record(
            name=name,
            tid=6,
            text_content="root1.usu.edu hostmaster@usu.edu 0 10800 3600 604800 3600",
            vid=None,
        )
        self.iface.add_dns_record(
            name=name, tid=1, ip_content=address, add_ptr=False, vid=None
        )
        self.iface.add_dns_record(
            name=name, tid=2, text_content="root1.usu.edu", vid=None
        )
        self.iface.add_dns_record(
            name=name, tid=2, text_content="root2.usu.edu", vid=None
        )
        self.iface.add_dns_record(
            name="www." + name, tid=5, text_content=name, vid=None
        )
    ...
    def do_create_user(self, arg):
        ...
        vals["password"] = self.get_password_from_user(
            "initial password for %s" % vals["username"]
        )
    ...
    def do_add_dns_record(self, arg):
        ...
        if typename in ["A", "AAAA"]:
            self.iface.add_dns_record(
                name=name, tid=tid, ip_content=content, add_ptr=add_ptr, vid=vid
            )  # , ttl=ttl )
        else:
            self.iface.add_dns_record(
                name=name, tid=tid, priority=priority, text_content=content, vid=vid
            )  # , ttl=ttl )
    ...
    def do_add_hosts(self, arg):
        ...
        expiration = datetime.datetime.today().replace(
    ...
    def do_assign_hosts_to_network(self, arg):
        ...
        for mac in re.findall(mac_regex, content):
            try:
                self.iface.change_registration(
                    old_mac=mac,
    ...
    def do_add_static_host(self, arg):
        vals = self.get_from_user(
            [
                ("hostname",),
                ("mac",),
                ("addr", "address or CIDR network"),
        ...
        mac = self.iface.register_host(
            ...
            do_validation=False,
            expires=expiration,
            add_host_to_my_group=add_host_to_my_group,
        )
    ...
    def do_add_dynamic_host(self, arg):
        vals = self.get_from_user(
            [
                ("hostname",),
                ("mac",),
                ("owners", "additional owners (space separated users or groups)"),
                ("pool", "pool id"),
            ],
            defaults={"pool": 1},
        )
    ...
    def do_add_network(self, arg):
        ...
        self.iface.add_network(
            network=network,
            name=name,
            description=desc,
            shared_network=shared_id,
            pool=pool_id,
            gateway=gateway,
        )
    ...
    def do_add_shared_network(self, arg):
        vals = self.get_from_user(
            [
                ("name",),
                ("networks", "networks (space-separated, CIDR)"),
                ("description",),
            ]
        )
    ...
    def do_add_structured_attribute_value(self, arg):
        vals = self.get_from_user(
            [("attribute",), ("value",), ("default",)], defaults={"default": False}
        )
        ...
        try:
            ...
            assert len(attr) == 1
            aid = attr[0]["id"]
        self.iface.add_structured_attribute_value(
            aid=aid, value=vals["value"], is_default=vals["default"]
        )
    ...
    def do_add_attribute_to_host(self, arg):
        ...
        if len(a) != 1:
            raise Exception(
                "attribute doesn't exist or is not unique: %s -> %s" % (attribute, a)
            )
        attribute = a[0]
    ...
    def do_add_dhcp_option_value(self, arg):
        vals = self.get_from_user(
            [("group",), ("option",), ("value", "value (precede hex with 0x)")]
        )
        arg_vals = {}
        ...
        print(
            "Preparing to call self.iface.add_dhcp_option_to_dhcp_group( %r )"
            % arg_vals
        )
    ...
    def do_assign_static_address(self, arg):
        ...
        vals = self.get_from_user(
            [
                ("mac",),
                ("net", "network (CIDR) or address"),
                ("name", "Name for A record (empty to skip)"),
            ]
        )
        mac = vals["mac"].strip()
        net = vals["net"].strip()
        ...
        if "/" in net:
            # mmm... network...
            ip = self.iface.assign_static_address(
                mac=mac, hostname=hostname, network=net
            )
        else:
            if openipam.iptypes.IP(net).version() == 4:
                ...
                if not addrs:
                    ...
                    raise Exception(
                        "Address %s not found.  Has the network been added?"
                    )
            ...
            ip = self.iface.assign_static_address(
                mac=mac, hostname=hostname, address=net
            )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            "Usage:\n\t %s URL\n\t\t where URL is of the form https://url.of.server:8443/api/"
            % sys.argv[0]
        )
        exit(1)
    url = str(sys.argv[1])
