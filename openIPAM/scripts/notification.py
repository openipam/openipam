...
for rowitem in notification_list:
    ...
    if rowitem["username"] in contactlist:
        ...
        contactlist["%(username)s" % rowitem] = {
            "rows": [row_heading, row_text],
            "is_static": rowitem["is_static"],
            "notifications": [rowitem["nid"]],
        }
...
for item in contactlist:
    try:
        ...
        if not email_re.search(user.email):
            raise Exception(
                "User %s has an invalid preferred email address (%s)"
                % (item, user.email)
            )
    except Exception as e:
        if contactlist[item]["is_static"]:
            failed_static.append(
                "%s: %s\n%s\n\n" % (e, item, "\n".join(contactlist[item]["rows"]))
            )
        else:
            failed_dynamic.append(
                "%s: %s\n%s\n\n" % (e, item, "\n".join(contactlist[item]["rows"]))
            )
        continue
        # print item
