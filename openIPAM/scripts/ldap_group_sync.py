...
for group in ipam_groups:
    ...
    for groupname in grouplist:
        ...
        ldap_data = ldap_interface._query(
            basedn="ou=banner,dc=auth,dc=usu,dc=edu",
            filter=ldap_filter,
            attrs=["sAMAccountName", "objectClass", "dn"],
        )
        for user in ldap_data:
            # print user
            if (
                "group" in user[1]["objectClass"]
            ):  # this is a sub-group, keep looking stuff up
    ...
    for uid in to_add:
        ...
        try:
            ipam_interface.add_user_to_group(
                uid=uid, gid=group["id"], permissions=str(interface.perms.OWNER)
            )
        except:
            print("Failed")
