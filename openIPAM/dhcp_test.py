...
class PacketGenerator( object ):
	...
	def __get_statics( self ):
		statics_q = self.obj.select( [self.obj.addresses, self.obj.networks.c.gateway], from_obj=self.obj.addresses.join(self.obj.networks, self.obj.addresses.c.address.op('<<')(self.obj.networks.c.network) ) )
	...
	def get_random_mac( self ):
		return "aa:aa:aa:%2x:%2x:%2x" % (random.randrange(0,256), random.randrange(0,256), random.randrange(0,256),)
	def get_gateways( self ):
		gateways_q = self.obj.select( [self.obj.networks.c.gateway] )
		gateways_q = gateways_q.where( self.obj.networks.c.gateway.op('<<')( '129.123.0.0/16' ) )
	...
	def GetNextDhcpPacket( self ):
		...
		invalid = random.random() < .1
		bound = random.random() < .75
		...
		type,packet = make_dhcp_packet( discover=discover, requested=address, bound=bound, mac=mac, gateway=gateway )
...
def make_dhcp_packet( mac, requested, gateway, discover=False, bound=True ):
	...
	packet.SetOption("parameter_request_list",[1,3,6,15,31,33,119,95,252,44,46,47,42,28,])
...
if __name__ == '__main__':
	...
	db_pool = processing.Pool( processes=NUM_WORKERS, initializer=dhcp_server.db_consumer, initargs=( db_requests, server.send_packet ) )
