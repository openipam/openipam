...
if auth.interfaces:
    ...
    if auth.internal_enabled:
        interfaces[
            auth.sources.INTERNAL
        ] = openipam.backend.auth.interfaces.InternalAuthInterface()
...
def update_password(username, password, old_password=None):
    ...
    return interface.update_password(
        username=username, password=password, old_password=old_password
    )
