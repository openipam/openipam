...
class InternalAuthInterface(BaseAuthInterface):
    ...
    def _search_internal(self, username):
        ...
        if not internal_auth_info:
            raise error.NotUser(
                "User does not exist in internal authentication source."
            )
    ...
    def __verify(self, username, password=False):
        ...
        return User(
            ...
            name=internal_auth_info["name"],
            source=auth.sources.INTERNAL,
            min_perms=user["min_permissions"],
            email=internal_auth_info["email"],
        )
    ...
    def create_user(self, username, password, name, email):
        ...
        return auth.dbi.create_internal_user(
            username=username, name=name, email=email, hash=hash_password(password)
        )
    ...
    def update_password(self, username, password, old_password=None):
        ...
        return auth.dbi.change_internal_password(
            id=user.uid, hash=hash_password(password)
        )
...
class LDAPInterface(BaseAuthInterface):
    ...
    def _search_ldap(self, username):
        ...
        result = self.__conn.search_st(
            self.__basedn,
            self.__scope,
            self.__filter % username,
        ...
        if len(result) == 1:
            ...
            return {"username": username, "name": display_name, "email": mail, "dn": dn}
        elif result:
            raise error.NotUnique()
        else:
    ...
    def __verify(self, username, password=(False,)):
        ...
        if "email" not in ldap_user or (
            "email" in ldap_user and not ldap_user["email"].strip()
        ):
            if auth.ldap_require_email:
                raise error.NoEmail()
            else:
        ...
        our_user = auth.dbi.get_users(
            username=ldap_user["username"], source=auth.sources.LDAP
        )
        ...
        if not len(our_user) and auth.ldap_auto_create:
            ...
            if not our_user:
                raise error.InsertFailed(
                    "New user from LDAP could not be inserted into the database"
                )
        elif len(our_user) > 1:
        ...
        return User(
            ...
            name=ldap_user["name"],
            source=auth.sources.LDAP,
            min_perms=our_user["min_permissions"],
