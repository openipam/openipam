...
class DBBaseInterface:
    ...
    def require_perms(self, permission, error_str=None):
        if not error_str:
            error_str = f"Insufficient Permissions (have: {self._min_perms}, need: {permission})"
    ...
    def __getattr__(self, name):
        ...
        if name[:4] == "get_":
            ...
            return fcn_wrapper(
                obj=self,
                fcn=self._execute_get,
                kwargs={"execute_get_function": obj},
                name=name,
            )
    ...
    def _require_perms_on_host(self, permission, mac, error_msg=None):
        ...
        if not self.has_min_perms(permission):
            host_perms = obj.perm_query(
                self._uid, self._min_perms, hosts=True, required_perms=permission
            )
            net_perms = obj.perm_query(
                self._uid, self._min_perms, networks=True, required_perms=permission
            )
            dom_perms = obj.perm_query(
                self._uid, self._min_perms, domains=True, required_perms=permission
            )

            # Find all hosts where the user has access via networks_to_groups
            net_perms = net_perms.join(
                obj.addresses,
                and_(
                    net_perms.c.nid == obj.addresses.c.network,
                    obj.addresses.c.mac == mac,
                ),
            )

            # Find all hosts where the user has access via hosts_to_groups
            host_perms = host_perms.join(
                obj.hosts,
                and_(host_perms.c.mac == obj.hosts.c.mac, obj.hosts.c.mac == mac),
            )

            # Find all hosts where the user has access via networks_to_groups
            dom_perms = dom_perms.join(
                obj.domains, dom_perms.c.did == obj.domains.c.id
            ).join(
                obj.hosts,
                and_(
                    obj.hosts.c.hostname.like("%." + obj.domains.c.name),
                    obj.hosts.c.mac == mac,
                ),
            )

            net_hosts = select(obj.addresses.c.mac).select_from(net_perms)
            group_hosts = select(obj.hosts.c.mac).select_from(host_perms)
            dom_hosts = select(obj.hosts.c.mac).select_from(dom_perms)
        ...
        if not self.has_min_perms(permission):
            ...
            if len(addr_record) > 1:
                raise error.InvalidArgument(
                    "'address' argument should only match a single address: %s"
                    % address
                )
            if addr_record:
            ...
            if not addr_record or not addr_record.mac:
                if not error_msg:
                    error_msg = f"need {perms} or greater on network containing {address}"
                raise error.InsufficientPermissions(error_msg)
            ...
            if host_perms is None or len(host_perms) == 0:
                if not error_msg:
                    error_msg = (
                        "need %s or greater network containing %s or host with mac %s"
                        % (perms, address, addr_record.mac)
                    )
                raise error.InsufficientPermissions(error_msg)
            return

    def _require_perms_on_net(
        self, permission, network=None, address=None, error_msg=None
    ):
        if not self.has_min_perms(permission):
            ...
            if not net_perms:
                if not error_msg:
                    error_msg = (
                        "need %s or greater on network %s or network containing %s"
                        % (perms, network, address)
                    )
                raise error.InsufficientPermissions(error_msg)
    ...
    def find_owners_of_host(self, mac, get_users=False):
        ...
        fromobject = obj.groups.join(
            obj.hosts_to_groups,
            and_(
                obj.hosts_to_groups.c.gid == obj.groups.c.id,
                obj.hosts_to_groups.c.mac == mac,
            ),
        )
        ...
        whereobject = obj.users_to_groups.c.permissions.op("|")(
        ...
        if get_users:
            ...
            fromobject = fromobject.join(
                obj.users, obj.users.c.id == obj.users_to_groups.c.uid
            )
            query = select(obj.users).select_from(fromobject).where(whereobject).distinct()
        else:
            query = select(obj.groups).select_from(fromobject).where(whereobject).distinct()
    ...
    def find_mac_from_lease(self, ip):
        ...
        query = select(obj.leases.c.mac).where(obj.leases.c.address == ip)
    ...
    def find_expiring_hosts(self):
        ...
        from_object = obj.hosts.join(
            obj.notifications_to_hosts,
            obj.notifications_to_hosts.c.mac == obj.hosts.c.mac,
        )
        from_object = from_object.join(
            obj.notifications,
            obj.notifications.c.id == obj.notifications_to_hosts.c.nid,
        )
        from_object = from_object.join(
            obj.hosts_to_groups,
            obj.hosts_to_groups.c.mac == obj.notifications_to_hosts.c.mac,
        )
        from_object = from_object.join(
            obj.users_to_groups, obj.users_to_groups.c.gid == obj.hosts_to_groups.c.gid
        )
        from_object = from_object.join(
            obj.users, obj.users_to_groups.c.uid == obj.users.c.id
        )
        from_object = from_object.outerjoin(
            obj.addresses, obj.hosts.c.mac == obj.addresses.c.mac
        )

        columns = (
            obj.hosts.c.mac,
            obj.hosts.c.hostname,
            obj.hosts.c.expires,
            obj.hosts.c.description,
    ...
    def _finalize_whereclause(self, whereclause):
        ...
        if len(whereclause) == 1:
            ...
            raise error.RequiredArgument(
                "_finalize_whereclause needs a list of criterion"
            )
    ...
    def _get_addresses(self, address=None, network=None, mac=None, pool=None):
        ...
        fromobject = obj.addresses.join(
            obj.networks, obj.networks.c.network == obj.addresses.c.network
        )

        query = select(
            obj.addresses,
            sqlalchemy.sql.func.netmask(obj.addresses.c.network).label("netmask"),
            obj.networks.c.gateway,
        ).select_from(fromobject)
    ...
    def _get_dhcp_options(self, gid=None, id=None, option=None):
        ...
        query = select(obj.dhcp_options).select_from(fromobject)
    ...
    def _get_dns_records(
        ...
        if not columns:
            columns = (
                obj.dns_records,
                obj.dns_types.c.name.label("type"),
                sqlalchemy.sql.func.coalesce(
                    obj.dns_records.c.text_content,
                    sqlalchemy.sql.expression.cast(
                        obj.dns_records.c.ip_content, sqlalchemy.types.VARCHAR
                    ),
                ).label("content"),
            )
            ...
            if type(content) is list:
                ...
                if validation.is_ip(content):
                    ...
                    if "%" in content:
                        ...
                        whereclause.append(
                            or_(
                                obj.dns_records.c.text_content == content,
                                obj.dns_records.c.text_content.like("%% %s" % content),
                            )
                        )
        ...
        if mac:
            ...
            if addresses:
                a_records = self._get_dns_records(
                    address=addresses, columns=columns
                ).distinct()
                union_foo.append(a_records.where(whereclause))

                ptr_names = [
                    openipam.iptypes.IP(i).reverseName()[:-1] for i in addresses
                ]
                if ptr_names:
                    ptrs = self._get_dns_records(
                        name=ptr_names, columns=columns
                    ).distinct()
                ...
                a_record_names = [
                    ...
                    for i in self._execute(
                        self._get_dns_records(address=addresses).with_only_columns(
                            [obj.dns_records.c.name]
                        )
                    )
                ]
                if a_record_names:
                    other_records = self._get_dns_records(
                        content=a_record_names, columns=columns
                    ).distinct()
                    union_foo.append(other_records.where(whereclause))
                    other_records = self._get_dns_records(
                        name=a_record_names, columns=columns
                    ).distinct()
                    union_foo.append(other_records.where(whereclause))
            ...
            if len(union_foo) > 2:
                ...
                query = select(*columns).select_from(obj.dns_records).where(False)

        else:
            from_object = obj.dns_records.join(
                obj.dns_types, obj.dns_records.c.tid == obj.dns_types.c.id
            )
            query = select(*columns).select_from(from_object)

            if whereclause is not True:
                query = query.where(whereclause)
            else:
                raise error.InvalidArgument(
                    "You're trying to retrieve all DNS records ... why?"
                )
    ...
    def _get_dns_types(self, typename=None, only_useable=False):
        ...
        if only_useable:
            query = query.where(
                and_(
                    not_(obj.dns_types.c.min_permissions == "00000000"),
                    obj.dns_types.c.min_permissions.op("&")(str(self._min_perms))
                    == obj.dns_types.c.min_permissions,
                )
            )
        if typename:
    ...
    def _get_domains(
        ...
        if self.has_min_perms(required_perms):
            ...
            if gid:
                query = query.join(
                    obj.domains_to_groups,
                    and_(
                        obj.domains.c.id == obj.domains_to_groups.c.did,
                        obj.domains_to_groups.c.gid == gid,
                    ),
                )
        else:
            domain_perms = obj.perm_query(
                self._uid,
                self._min_perms,
                domains=True,
                gid=gid,
            ...
            query = domain_perms.join(
                obj.domains, obj.domains.c.id == domain_perms.c.did
            )
        ...
        if contains:
            ...
            query = query.order_by(
                sqlalchemy.sql.func.length(obj.domains.c.name).desc()
            )
        if not show_reverse:
    ...
    def _get_expiration_types(self):
        ...
        query = (
            select(obj.expiration_types.c.id, obj.expiration_types.c.expiration)
            .where(obj.expiration_types.c.min_permissions <= str(self._min_perms))
        )
    ...
    def _get_guest_tickets(self, ticket=None, uid=None):
        ...
        if not ticket and not uid:
            raise error.RequiredArgument(
                "Must specify at least one of name or uid to get guest tickets"
            )
        ...
        query = select(
            ...
            (
                and_(
                    obj.guest_tickets.c.starts <= sqlalchemy.sql.func.now(),
                    obj.guest_tickets.c.ends > sqlalchemy.sql.func.now(),
                )
            ).label("valid"),
        )
    ...
    def _get_users_to_groups(self, uid=None, gid=None):
        ...
        query = select(obj.users_to_groups).select_from(obj.users_to_groups)
        if uid:
    ...
    def _get_groups(
        ...
        if uid:
            ...
            fromobj = obj.groups.join(
                obj.users_to_groups,
                and_(
                    and_(
                        obj.groups.c.id == obj.users_to_groups.c.gid,
                        obj.users_to_groups.c.uid == uid,
                    ),
                    obj.users_to_groups.c.permissions.op("|")(
                        obj.users_to_groups.c.host_permissions
                    ).op("&")(str(additional_perms))
                    == str(additional_perms),
                ),
            )
        else:
            fromobj = obj.groups

        query = select(obj.groups).select_from(fromobj)
        ...
        if name:
            if "%" in name:
                query = query.where(
                    sqlalchemy.sql.func.lower(obj.groups.c.name).like(name.lower())
                )
            else:
                query = query.where(
                    sqlalchemy.sql.func.lower(obj.groups.c.name) == name.lower()
                )
        if ignore_usergroups:
            query = query.where(
                not_(sqlalchemy.sql.func.lower(obj.groups.c.name).like("user_%"))
            )
    ...
    def _get_hosts(
        ...
        if endmac and not mac:
            raise error.RequiredArgument(
                f"Beginning of range not specified: mac: {mac} endmac: {endmac}"
            )

        if endmac and only_statics:
            raise error.InvalidArgument(
                "Specifying endmac and only_statics not supported"
            )

        if only_dynamics and only_statics:
            raise error.RequiredArgument(
                "Cannot specify both only_dynamics and only_statics"
            )

        if backend.enable_gul:
            gul_byaddr_fromclause = obj.addresses.join(
                obj.gul_recent_arp_byaddress,
                obj.gul_recent_arp_byaddress.c.address == obj.addresses.c.address,
            )
            gul_addr_seen_column = sqlalchemy.sql.func.max(
                obj.gul_recent_arp_byaddress.c.stopstamp
            ).label("address_seen")
            gul_byaddr_columns = (obj.addresses.c.mac, gul_addr_seen_column)
            # query = subquery( 'label', columns, whereclause, from_obj = fromclause )
            gul_by_addr_subq = (
            select(*gul_byaddr_columns)          # 1. Unpack columns with *
            .select_from(gul_byaddr_fromclause)
            .group_by(obj.addresses.c.mac)
            .subquery("gul_byaddress_subq")      # 2. Use .subquery() instead of .alias()
        )
        ...
        if funky_ordering:
            columns.append(
                sqlalchemy.sql.func.length(obj.hosts.c.hostname).label("len")
            )
        ...
        if ip is not None:
            if type(ip) is list:
                ...
                whereclause.append(
                    or_(obj.addresses.c.address == ip, obj.leases.c.address == ip)
                )
        if only_statics:
        ...
        if mac is not None:
            if endmac is not None:
                whereclause.append(
                    and_(obj.hosts.c.mac >= mac, obj.hosts.c.mac <= endmac)
                )
            else:
        ...
        if expiring:
            whereclause.append(
                obj.hosts.c.expires
                < sqlalchemy.sql.func.now() + text("interval '%d days'" % int(expiring))
            )

            # Check permissions and generate the query
        hosts = obj.hosts
        hosts = hosts.outerjoin(
            obj.dhcp_groups, obj.hosts.c.dhcp_group == obj.dhcp_groups.c.id
        )
        ...
        if only_dynamics:
            hosts = hosts.join(
                obj.hosts_to_pools, obj.hosts_to_pools.c.mac == obj.hosts.c.mac
            )
        ...
        if gid:
            hosts = hosts.join(
                obj.hosts_to_groups,
                and_(
                    obj.hosts.c.mac == obj.hosts_to_groups.c.mac,
                    obj.hosts_to_groups.c.gid == gid,
                ),
            )
        ...
        if uid:
            ...
            hosts = hosts.join(
                obj.users_to_groups,
                and_(
                    obj.users_to_groups.c.gid == obj.hosts_to_groups.c.gid,
                    and_(
                        obj.users_to_groups.c.uid == uid,
                        obj.users_to_groups.c.permissions.op("|")(
                            obj.users_to_groups.c.host_permissions
            ...
            if not self._min_perms & required_perms == required_perms:
                ...
                hosts = [
                    hosts.join(host_perms, host_perms.c.mac == obj.hosts.c.mac),
        ...
        if namesearch:
            ...
            dns_q = a_tbl.outerjoin(
                ...
                and_(
                    cname_where,
                    and_(
                        a_tbl.c.name == cname_tbl.c.text_content, cname_tbl.c.tid == 5
                    ),
                ),
            )

            cname_from = a_tbl.join(
                cname_tbl,
                and_(
                    cname_where,
                    and_(
            ...
            dns_hosts_from = obj.hosts.join(
                obj.addresses, obj.addresses.c.mac == obj.hosts.c.mac
            )
            ...
            a_q = select(
                ...
                dns_hosts_from.join(
                    a_tbl, and_(a_tbl.c.ip_content == obj.addresses.c.address, a_where)
                )
            )
            ...
            cname_q = select(
                ...
                dns_hosts_from.join(
                    cname_from, a_tbl.c.ip_content == obj.addresses.c.address
                ),
            )

            hosts_q = select(obj.hosts.c.mac).select_from(obj.hosts).where(hosts_where)

            dns_q = union(a_q, cname_q, hosts_q).subquery("namesearch_macs")
        ...
        if type(hosts) is list:
            if backend.enable_gul:
                ...
                for i in hosts:
                    i = i.outerjoin(
                        obj.gul_recent_arp_bymac,
                        obj.hosts.c.mac == obj.gul_recent_arp_bymac.c.mac,
                    )
                    i = i.outerjoin(
                        gul_by_addr_subq, obj.hosts.c.mac == gul_by_addr_subq.c.mac
                    )
                    i = select(*columns).select_from(i).where(whereclause).distinct()
                    newhosts.append(i)
                hosts = newhosts
            else:
                hosts = [
                    select(*columns).select_from(i).where(whereclause).distinct()
                    for i in hosts
                ]
            hosts = union(*hosts).subquery("union_hosts")
        else:
            if backend.enable_gul:
                # hosts=hosts.join()
                hosts = hosts.outerjoin(
                ...
                hosts = select(*columns).select_from(hosts).distinct()
            else:
                hosts = select(*columns).select_from(hosts).distinct()
        ...
        if self.has_min_perms(required_perms):
            ...
            # think about it
            if funky_ordering:
                hosts = hosts.order_by(hosts.c.len.desc()).order_by(obj.hosts.c.hostname.desc())

        return hosts

    def _find_permissions_for_objects_query(
        self,
        objects,
        primary_table,
        primary_key,
        ...
        if objects and (
            type(objects[0]) is dict
            or type(objects[0] is sqlalchemy.engine.base.RowProxy)
        ):
        ...
        fromobj = bridge_table.join(
            primary_table,
            and_(foreign_key == primary_key, primary_key.in_(objects_list)),
        ).outerjoin(
            obj.users_to_groups,
            and_(
                obj.users_to_groups.c.gid == bridge_table.c.gid,
                obj.users_to_groups.c.uid == self._uid,
            ),
        )

        permissions_col = sqlalchemy.sql.func.coalesce(
            sqlalchemy.sql.func.bit_or(obj.users_to_groups.c.permissions).op("|")(
                str(self._min_perms)
            ),
            str(self._min_perms),
        )
        ...
        if primary_key == obj.hosts.c.mac:
            host_permissions_col = sqlalchemy.sql.func.coalesce(
                sqlalchemy.sql.func.bit_or(obj.users_to_groups.c.host_permissions),
                str(self._min_perms),
            )
    ...
    def _find_permissions_for_objects(
        self,
        objects,
        primary_table,
        primary_key,
        ...
        query = select(*columns).select_from(fromobj).group_by(primary_key)
        if alternate_perms_key is not None:
        ...
        perms_key_name = (
            alternate_perms_key.name
            if alternate_perms_key is not None
            else primary_key.name
        )

        for row in results:
            permissions[getattr(row, perms_key_name)] = row.permissions
    ...
    def find_permissions_for_hosts(self, hosts, alternate_perms_key=None):
        ...
        permissions_col, fromobj = self._find_permissions_for_objects_query(
            objects=hosts,
            primary_table=obj.hosts,
            primary_key=primary_key,
            bridge_table=obj.hosts_to_groups,
            foreign_key=obj.hosts_to_groups.c.mac,
            alternate_perms_key=alternate_perms_key,
        )
        if permissions_col is None or fromobj is None:
            return [{}]

        dom_u2g = obj.users_to_groups.alias("domain_users_to_groups")
        dom_u2g2d = dom_u2g.join(
            obj.domains_to_groups,
            and_(
                dom_u2g.c.gid == obj.domains_to_groups.c.gid, dom_u2g.c.uid == self._uid
            ),
        )

        fromobj = fromobj.outerjoin(
            obj.domains, obj.hosts.c.hostname.like("%." + obj.domains.c.name)
        )
        ...
        columns = [
            primary_key,
            permissions_col.label("permissions"),
            dom_permissions_col.label("meh"),
        ]
    ...
    def find_permissions_for_domains(self, domains, alternate_perms_key=None):
        ...
        return self._find_permissions_for_objects(
            objects=domains,
            primary_table=obj.domains,
            primary_key=obj.domains.c.id,
            bridge_table=obj.domains_to_groups,
            foreign_key=obj.domains_to_groups.c.did,
            alternate_perms_key=alternate_perms_key,
        )
    ...
    def find_domain_permissions_for_fqdns(self, names):
        ...
        for name in names:
            ...
            if (
                name in domain_name_perms
                or first_level_domain_name in domain_name_perms
            ):
                perms_to_add = (
                    domain_name_perms[name]
                    if name in domain_name_perms
                    else domain_name_perms[first_level_domain_name]
                )
    ...
    def _get_hosts_to_pools(self, mac=None):
        ...
        fromobj = obj.hosts_to_pools.join(
            obj.pools, obj.hosts_to_pools.c.pool_id == obj.pools.c.id
        )
        query = select(
            obj.hosts_to_pools, obj.pools.c.name, obj.pools.c.description
        ).select_from(fromobj)
    ...
    def _get_leases(self, address=None, mac=None, show_expired=True):
        ...
        columns = [
            obj.leases,
            (obj.leases.c.ends < sqlalchemy.sql.func.now()).label("expired"),
        ]
    ...
    def _get_internal_auth(self, uid):
        ...
        return select(obj.internal_auth).where(obj.internal_auth.c.id == uid)
    ...
    def _get_networks(
        ...
        if self.has_min_perms(required_perms):
            ...
            net_perms = obj.perm_query(
                self._uid, self._min_perms, networks=True, required_perms=required_perms
            )
            query = net_perms.join(
                obj.networks, obj.networks.c.network == net_perms.c.nid
            )

        if gid:
            query = query.join(
                obj.networks_to_groups,
                and_(
                    obj.networks.c.network == obj.networks_to_groups.c.nid,
        ...
        query = select(
            obj.networks,
            sqlalchemy.sql.func.netmask(obj.networks.c.network).label("netmask"),
        ).select_from(query)
    ...
    def _get_networks_to_groups(self, nid=None, gid=None):
        ...
        if self._min_perms & perms.READ is perms.READ:
            ...
            relation = relation.where(
                and_(
                    obj.networks_to_groups.c.nid == nid,
                    obj.networks_to_groups.c.gid == gid,
                )
            )
        else:
    ...
    def is_disabled(self, mac=None, address=None):
        ...
        if (not address and not mac) or (address and mac):
            raise error.RequiredArgument(
                "You must specify exactly one of mac or address: address=%s, mac=%s"
                % (address, mac)
            )
        ...
        if mac:
            ...
            query = select(
                obj.disabled
            ).select_from(obj.disabled.join(
                    obj.leases, obj.leases.c.mac == obj.disabled.c.mac
                )
            )
    ...
    def _get_users(self, uid=None, username=None, source=None, gid=None):
        ...
        if gid:
            ...
            query = obj.users.join(
                obj.users_to_groups,
                and_(
                    obj.users.c.id == obj.users_to_groups.c.uid,
                    obj.users_to_groups.c.gid == gid,
                ),
            )
        ...
        if username:
            if "%" in username:
                query = query.where(
                    sqlalchemy.sql.func.lower(obj.users.c.username).like(
                        username.lower()
                    )
                )
            else:
    ...
    def _get_gul_recent_arp_byaddress(self, address):
        ...
        if type(address) is list:
            ...
            raise error.InvalidArgument(
                "expecting string or list for address: %r" % address
            )

        fmt = 'FMDD"d "FMHH24"h "FMMI"m "FMSS"s"'
        last_seen = sqlalchemy.sql.func.to_char(
            sqlalchemy.sql.func.now() - obj.gul_recent_arp_byaddress.c.stopstamp, fmt
        ).label("last_seen")

        query = select(obj.gul_recent_arp_byaddress, last_seen).where(where)
    ...
    def _get_gul_recent_arp_bymac(self, mac):
        ...
        fmt = 'FMDD"d "FMHH24"h "FMMI"m "FMSS"s"'
        last_seen = sqlalchemy.sql.func.to_char(
            sqlalchemy.sql.func.now() - obj.gul_recent_arp_bymac.c.stopstamp, fmt
        ).label("last_seen")

        query = select(obj.gul_recent_arp_bymac, last_seen).where(
            obj.gul_recent_arp_bymac.c.mac == mac
        )
...
class DBInterface(DBBaseInterface):
    ...
    def _assign_ip6_address(
        self, mac, network, dhcp_server_id=0, use_lowest=False, is_server=False
    ):
        ...
        if network.prefixlen() == 128:
            ...
            q = select(
                (obj.addresses.c.address + 1).label("next")
            ).select_from(obj.addresses).where(obj.addresses.c.address.op("<<")(str(network)))
            sub_q = sqlalchemy.sql.exists(
                whereclause=and_(
                    a.c.address == obj.addresses.c.address + 1,
                    (a.c.address + 1).op("<<")(str(network)),
                )
            )
            q = q.where(~sub_q)
            ...
            if not results:
                raise Exception(
                    "Did not find an ip6 address?! network: %s prefixlen: %s mac: %s"
                    % (network, network.prefixlen(), mac)
                )
            address = results[0][0]
        else:
            macaddr = int("0x" + re.sub(mac, "[^0-9A-Fa-f]+", ""))
            lastbits = (macaddr & 0xFFFFFF) ^ (macaddr >> 24) | (
                random.getrandbits(24) << 24
            )
        ...
        network = self._execute(
            select(obj.networks.c.network).select_from(obj.networks).where(
                obj.networks.c.network.op(">>")(str(address))
            )
        )[0][0]
    ...
    def _do_delete(self, table, where):
        if where is None or where is True or where == "":
            raise error.InvalidArgument(
                'You just tried to delete everything in the "%s" table.  where=%s'
                % (str(table.name), str(where))
            )
        ...
        try:
            ...
            if (
                table.name == "disabled"
                or "changed" in table.c
                or "changed_by" in table.c
            ):
    ...
    def add_address(self, address, network, mac=None, pool=None, reserved=False):
        ...
        if addr.family == 6 and not backend.allow_ipv6:
            raise error.InvalidArgument(
                "IPv6 support is disabled in backend configuration."
            )
        del addr
    ...
    def update_address(self, address, mac=None, pool=None):
        ...
        if len(c_address) != 1:
            raise error.NotFound(
                f"Address {address} does not exist ({c_address})"
            )
        ...
        if not self.has_min_perms(perms.DEITY):
            if c_address.reserved:
                raise error.InsufficientPermissions(
                    "You must be a superuser to alter reserved addresses (%s)" % address
                )

                # check to see if we are allowed to rob this pool
            if (
                c_address.pool is not None
                and c_address.pool not in backend.assignable_pools
            ):
                raise error.InsufficientPermissions(
                    "Only a superuser can alter addresses in this pool: %s" % c_address
                )
    ...
    def add_pool_to_group(self, pool, gid):
        ...
        return self._do_insert(
            table=obj.pools_to_groups, values={"pool": pool, "gid": gid}
        )
    ...
    def add_host_to_pool(self, mac, pool_id):
        ...
        if not self.has_min_perms(perms.ADD):
            ...
            pool_perms = obj.perm_query(
                ...
                self._min_perms,
                pools=True,
                required_perms=perms.ADD,
                do_subquery=False,
                andwhere=obj.pools_to_groups.c.pool == pool_id,
            )
            pools = self._execute(pool_perms)
            if not pools:
                raise error.InsufficientPermissions(
                    "ADD permission required over pool id %s" % pool_id
                )

        return self._do_insert(
            table=obj.hosts_to_pools, values={"mac": mac, "pool_id": pool_id}
        )
    ...
    def add_structured_attribute_value(self, aid, value, is_default=False):
        ...
        if not attr.structured:
            raise error.InvalidArgument(
                "aid specified is not a structured attribute: %s" % attr
            )

        return self._do_insert(
            table=obj.structured_attribute_values,
            values={"aid": aid, "value": value, "is_default": is_default},
        )
    ...
    def add_structured_attribute_to_host(self, mac, avid):
        ...
        if len(attr_value) != 1:
            raise error.InvalidArgument(
                "Structured attribute value non-existent or not unique: %s" % attr_value
            )

        return self._do_insert(
            table=obj.structured_attributes_to_hosts, values={"mac": mac, "avid": avid}
        )
    ...
    def del_structured_attribute_to_host(self, mac, avid):
        ...
        where = and_(
            obj.structured_attributes_to_hosts.c.mac == mac,
            obj.structured_attributes_to_hosts.c.avid == avid,
        )
    ...
    def add_freeform_attribute_to_host(self, mac, aid, value):
        ...
        if attr.structured:
            raise error.InvalidArgument(
                "aid specified is not a freeform attribute: %s" % attr
            )

        return self._do_insert(
            table=obj.freeform_attributes_to_hosts,
            values={"mac": mac, "aid": aid, "value": value},
        )
    ...
    def del_freeform_attribute_to_host(self, mac, aid, value):
        ...
        where = and_(
            obj.freeform_attributes_to_hosts.c.mac == mac,
            obj.freeform_attributes_to_hosts.c.aid == aid,
    ...
    def add_dns_record(
        ...
        if tid == 5:  # CNAME
            ...
            if records:
                raise error.InvalidArgument(
                    "Trying to create CNAME record while other records exist: %r"
                    % records
                )
        else:  # not CNAME
            ...
            if records:
                raise error.InvalidArgument(
                    "Trying to create record while CNAME record exists: %r" % records
                )
        ...
        try:
            ...
            if not domains:
                ...
                if tid == 12 and domains:
                    ...
                    raise error.InsufficientPermissions(
                        "Insufficient permissions to access domain containing %s" % name
                    )

            if domains[0].type == "SLAVE":
                raise error.InvalidArgument(
                    "Cannot create name %s: not authoritative for domain %s"
                    % (name, domains[0].name)
                )
            ...
            if tid == 1 or tid == 28:
                ...
                if tid == 1 and ip.version() != 4:
                    raise Exception(
                        "A record must have ip4 address: name: %s tid: %s address: %s"
                        % (name, tid, ip_content)
                    )
                ...
                if add_ptr:
                    ...
                    self.add_dns_record(
                        name=ptrname, text_content=name, tid=12, ttl=ttl, vid=vid
                    )
    ...
    def add_dhcp_group(self, name, description):
        ...
        self.require_perms(perms.DEITY)
        return self._do_insert(
            obj.dhcp_groups, {"name": name, "description": description}
        )
    ...
    def add_dhcp_option_to_dhcp_group(self, gid, oid, value, is_hex=False):
        ...
        if len(dhcp_option) != 1:
            raise Exception(
                "dhcp_option %d does not exist or not unique: %r" % (oid, dhcp_option)
            )
    ...
    def add_domain_to_group(self, did, gid):
        ...
        return self._do_insert(
            table=obj.domains_to_groups, values={"did": did, "gid": gid}
        )
    ...
    def add_group(self, name, description=None):
        ...
        return self._do_insert(
            table=obj.groups, values={"name": name, "description": description}
        )
    ...
    def __find_next_mac(self, mac):
        ...
        h = obj.hosts.alias("h")
        q = select(
            (obj.hosts.c.mac + 1).label("next")
        ).select_from(obj.hosts).where(sqlalchemy.sql.func.trunc(obj.hosts.c.mac) == oui)
        sub_q = sqlalchemy.sql.exists(
            whereclause=and_(
                h.c.mac == obj.hosts.c.mac + 1,
                sqlalchemy.sql.func.trunc(h.c.mac + 1) == oui,
            )
        )
        q = q.where(~sub_q)
        q = q.limit(1).order_by("next")
        ...
        if not results:
            raise Exception(
                f"Did not find a usable MAC address?! mac: {mac} (oui: {oui})"
            )
        address = results[0][0]
        return address
    ...
    def add_host(self, mac, hostname, description=None, dhcp_group=None, expires=None):
        ...
        if re.search(r"\.arpa$", hostname):
            raise error.InvalidArgument(
                "hostname (%s) appears to be in a reverse-lookup domain" % hostname
            )
        ...
        try:
            ...
            if self.has_min_perms(perms.DEITY):
                ...
                raise error.InsufficientPermissions(
                    "User %s doesn't have domain access to add host %s"
            ...
            host = self.get_hosts(
                hostname=hostname, show_expired=True, show_active=False
            )
            dns = self.get_dns_records(name=hostname)
            if host:
                raise error.AlreadyExists(
                    "Host with name %s already exists.  Please delete it first."
                    % hostname,
                    hostname=hostname,
                )
            if dns:
                raise error.AlreadyExists(
                    "DNS record(s) with name %s already exist(s).  Please delete first."
                    % hostname,
                    hostname=hostname,
                )
    ...
    def add_host_to_group(self, mac, gid=None, group_name=None):
        ...
        if not gid:
            if not group_name:
                raise error.RequiredArgument(
                    "Must pass exactly one of gid or group_name"
                )
        ...
        try:
            if self.get_hosts_to_groups(mac=mac):
                ...
                self._require_perms_on_host(
                    ...
                    mac=mac,
                    error_msg="Couldn't add host %s to group %s, %s"
                    % (mac, gid, group_name),
                )

                # They have permission ... do the insert
            result = self._do_insert(
                table=obj.hosts_to_groups, values={"mac": mac, "gid": gid}
            )
    ...
    def assign_static_address(self, mac, hostname=None, network=None, address=None):
        ...
        ipv4 = (address and address.version() == 4) or (
            network and network.version() == 4
        )
        ...
        try:
            ...
            if network:
                ...
                if ipv4:
                    query = query.where(
                        obj.addresses.c.address.op("<<")(str(network))
                    ).order_by(obj.addresses.c.address)
                else:
            ...
            if not addresses:
                ...
                if ipv4:
                    ...
                    from_object = obj.addresses.outerjoin(
                    ...
                    query = query.where(
                        obj.addresses.c.pool.in_(backend.assignable_pools)
                    )
    ...
    def release_static_address(self, address, pool=False):
        ...
        if not addresses:
            raise error.NotFound(
                "No addresses returned in release_static_address for address %s"
                % address
            )
        ...
        if not self.has_min_perms(perms.DEITY):
            ...
            if not host:
                raise error.NotFound(
                    "No host found for MAC %s in release_static_address" % mac
                )
        ...
        if not address:
            raise error.RequiredArgument(
                "address is required in release_static_address"
            )
    ...
    def register_host(
        ...
        if pool is not None and (network or address):
            raise error.InvalidArgument(
                "Cannot assign a pool and an address: pool:%s,network:%s,address:%s"
                % (pool, network, address)
            )

        if add_host_to_my_group and owners:
            raise error.NotImplemented(
                "add_host_to_my_group must be False if owners is specified"
            )
        ...
        try:
            ...
            if pool is None:
                ...
                address = self.assign_static_address(
                    mac=mac, hostname=hostname, network=network, address=address
                )
            ...
            if not add_host_to_my_group:
                if not owners:
                    raise error.InvalidArgument(
                        "Must specify owners if add_host_to_my_group is false."
                    )
    ...
    def update_network(self, network, new_network=None, pool=False, **kw):
        ...
        if new_network:
            ...
            if not ip4 and net.prefixlen() != 64:
                raise Exception(
                    "Are you really trying to allocate a network that isn't a /64?"
                )
            if pool and not ip4:
        ...
        try:
            if new_network:
                query = select(
                    obj.networks.c.network
                ).where(
                    or_(
                        obj.networks.c.network.op("<<=")(new_network),
                        obj.networks.c.network.op(">>")(new_network),
                    )
                )
                result = self._execute(query)
            ...
            result = self._do_update(
                table=obj.networks, where=obj.networks.c.network == network, values=kw
            )
            ...
            if new_network:
                ...
                if ip4:
                    ...
                    for address in invalid:
                        self.add_address(
                            address=address, network=network, reserved=True
                        )
    ...
    def add_network(
        ...
        try:
            query = select(
                ...
                or_(
                    obj.networks.c.network.op("<<=")(network),
                    obj.networks.c.network.op(">>")(network),
                )
            )
            result = self._execute(query)
            ...
            if ip4:
                ...
                for address in net:
                    if (address not in invalid) or (net.prefixlen() >= 31):
                        ...
                        if pool is False:
                            ...
                            addr_pool = pool
                        self.add_address(
                            address=str(address), network=network, pool=addr_pool
                        )
                    else:
                        self.add_address(
                            address=str(address),
                ...
                for (
                    ...
                    invalid
                ):  # we do sparse addressing for ipv6.  This is very important.
    ...
    def add_network_to_group(self, nid, gid):
        ...
        return self._do_insert(
            table=obj.networks_to_groups, values={"nid": nid, "gid": gid}
        )
    ...
    def add_notification_to_host(self, nid, mac):
        ...
        try:
            if self.get_hosts_to_groups(mac=mac):
                ...
                # They have permission ... do the insert
            result = self._do_insert(
                table=obj.notifications_to_hosts, values={"nid": nid, "mac": mac}
            )
    ...
    def add_shared_network(self, name, description=None):
        ...
        return self._do_insert(
            table=obj.shared_networks, values={"name": name, "description": description}
        )
    ...
    def del_dns_record(self, rid=None, did=None, mac=None):
        ...
        if rid is not None and did is not None:
            raise error.InvalidArgument(
                f"del_dns_records only accepts one of (did={did}, rid={rid})"
            )
        ...
        if did is not None:
            ...
            if not records:
                raise error.NotFound(
                    "Couldn't delete DNS record id %s because it could not be found."
                    % rid
                )
            ...
            if not mac:
                ...
                if Perms(id_perms[rid]) & perms.DELETE != perms.DELETE:
                    raise error.InsufficientPermissions(
                        "Insufficient permissions to delete DNS record %s %s"
                        % (rid, record.name)
                    )
            else:
    ...
    def del_domain_to_group(self, did, gid):
        ...
        where = and_(
            obj.domains_to_groups.c.did == did, obj.domains_to_groups.c.gid == gid
        )
    ...
    def del_dhcp_dns_record(self, name=None, ip=None, network=None):
        ...
        if (bool(name) + bool(ip) + bool(network)) != 1:
            raise error.RequiredArgument(
                "Specify exactly one of name or IP address or network"
            )
    ...
    def del_guest_ticket(self, ticket):
        ...
        if not self.has_min_perms(perms.DEITY):
            ...
            if my_ticket[0].uid != self._uid:
                raise error.InsufficientPermission(
                    "Cannot delete another person's ticket"
                )
        else:
            my_ticket = self.get_guest_tickets(ticket=ticket)
    ...
    def del_host_to_group(self, mac, gid=None, group_name=None):
        ...
        self._require_perms_on_host(
            permission=perms.ADMIN,
            mac=mac,
            error_msg="Cannot delete host to group relation for host %s in group %s, %s"
            % (mac, gid, group_name),
        )
        ...
        if not gid and not group_name:
            ...
            self.require_perms(
                perms.DEITY,
                "You do not have permission to delete multiple host to group relations",
            )
    ...
    def del_network(self, network):
        ...
        try:
            ...
            if len(nets) != 1:
                raise error.NotFound(
                    "%d networks found matching %s -- not deleting"
                    % (len(nets), network)
                )

                # Delete all addresses that were in this network
            self._do_delete(
                table=obj.addresses, where=obj.addresses.c.address.op("<<")(network)
            )
    ...
    def del_network_to_group(self, nid, gid):
        ...
        where = and_(
            obj.networks_to_groups.c.nid == nid, obj.networks_to_groups.c.gid == gid
        )
    ...
    def del_notification_to_host(self, id=None, mac=None):
        ...
        if id and not self.has_min_perms(perms.DEITY):
            raise error.InsufficientPermissions(
                "Must be DEITY to specify ID of notification on host to remove"
            )

            # FIXME: do we need to do this if here? or just the require permissions
        if (
            not self.has_min_perms(perms.DEITY)
            and mac
            and self.get_hosts_to_groups(mac=mac)
        ):
            self._require_perms_on_host(permission=perms.ADMIN, mac=mac)
        ...
        if id:
            ...
            raise error.RequiredArgument(
                "Must specify exactly one of id or mac to del_notification_to_host"
            )
    ...
    def make_notifications_for_host(self, mac, expires):
        ...
        if not self.has_min_perms(perms.DEITY):
            self._require_perms_on_host(
                permission=perms.MODIFY,
                mac=mac,
                error_msg="Could not make_notifications_for_host(%s, %s)"
                % (mac, expires),
            )
    ...
    def update_host(
        ...
        results = self._do_update(
            table=obj.hosts, where=obj.hosts.c.mac == old_mac, values=values
        )

        # If we change expires, make sure notifications on the host are up-to-date
        if expires:
            self.make_notifications_for_host(
                mac=(mac if mac else old_mac), expires=expires
            )
    ...
    def change_registration(
        ...
        if address and network:
            ...
            if address in network:
                ...
                raise error.InvalidArgument(
                    "The address %s does not belong to the network %s"
                    % (address, str(network))
                )
        ...
        try:
            ...
            if not old_host:
                raise error.NotFound(
                    "change_registration could not find the host %s to update" % old_mac
                )
            old_host = old_host[0]
            ...
            if len(tmp_pool) > 1:
                raise Exception(
                    "Host has multiple pools assigned, cannot use register_host()"
                )
            ...
            if pool is not None and old_pool is None:
                ...
                for record in dns_records:
                    if record.tid == 1:  # A
                        if (
                            record.name != old_host.hostname
                            or openipam.iptypes.IP(record.ip_content)
                            != former_address
                        ):
                            nonstandard_records.append(record)
                    elif record.tid == 12:  # PTR
                        if (
                            record.name != former_address.reverseName()[:-1]
                ...
                self.update_host(
                    ...
                    description=description,
                    expires=expires,
                    expiration_format=expiration_format,
                    dhcp_group=dhcp_group,
                )
                ...
                if address or network:
                    ...
                    if not host_addresses:
                        raise error.NotFound(
                            "Couldn't find address(es) to release for this host"
                        )
    ...
    def set_owners_for_host(self, mac, owner_ids=None, owner_names=None):
        ...
        old_owner_ids = {int(row.gid) for row in old_owners}
    ...
    def change_address(self, mac, old_address, address=None, network=None):
        if (address and network) or (not address and not network):
            raise error.InvalidArgument(
                "Please specify exactly one of address(%s) and network(%s)"
                % (address, network)
            )
        ...
        try:
            ...
            self._do_update(
                ...
                where=obj.dns_records.c.ip_content == old_address,
                values=values,
            )

            # FIXME: Find the old PTR, add an equivalent one
            old_ptr = self.get_dns_records(
                name=openipam.iptypes.IP(old_address).reverseName()[:-1], typename="PTR"
            )
            if len(old_ptr) != 1:
            ...
            self.add_dns_record(
                name=openipam.iptypes.IP(new_address).reverseName()[:-1],
                tid=old_ptr.tid,
                text_content=old_ptr.text_content,
            )
    ...
    def update_dns_record(
        ...
        if not mac:
            ...
            self._require_perms_on_host(
                permission=perms.MODIFY,
                mac=mac,
                error_msg="Insufficient permissions to update DNS records for MAC %s"
                % mac,
            )
        ...
        if (old_name and not name) or (not old_name and name):
            ...
            raise error.RequiredArgument(
                "If old_name or name are specified, both must be given."
            )
        ...
        try:
            if old_address:
                ...
                dnswhere = and_(
                    obj.dns_records.c.ip_content == old_address,
                    obj.dns_records.c.name == old_name,
                )

                if address:
                    dnswhere = and_(
                        dnswhere, obj.dns_records.c.ip_content == old_address
                    )
                ...
                result = self._do_update(
                    table=obj.dns_records,
                    where=obj.dns_records.c.name == ptrname,
                    values=values,
                )
            else:
                raise error.NotImplemented()
    ...
    def update_group(self, gid, name=None, description=None):
        ...
        return self._do_update(
            table=obj.groups, where=obj.groups.c.id == gid, values=values
        )
    ...
    def disable_host(self, mac, reason=None):
        ...
        return self._do_insert(
            table=obj.disabled, values={"mac": mac, "reason": reason}
        )
    ...
    def delete_hosts(self, hosts=None):
        ...
        try:
            for mac in hosts:
                if not mac:
                    raise error.InvalidArgument(
                        f"Invalid MAC address: {mac} in host list {hosts}"
                    )
                self.del_host(mac=mac)
            self._commit()
        except Exception:
...
class DBAuthInterface(DBInterface):
    ...
    def add_user(self, username, source, min_perms=None):
        ...
        try:
            ...
            group_query = self.add_group(
                "user_%s" % username, "Default group for this user"
            )
            ...
            self.add_user_to_group(
                uid=uid, gid=backend.db_default_group_id, permissions=str(perms.ADD)
            )
    ...
    def change_internal_password(self, id, hash):
        ...
        return self._do_update(
            table=obj.internal_auth,
            where=obj.internal_auth.c.id == id,
            values={"hash": hash},
        )
...
class DBDHCPInterface(DBInterface):
    ...
    def update_or_create_lease_and_expire_conflicting(
        ...
        try:
            ...
            q = select(
                ...
                or_(del_cond, update_cond, lease_cond, obj.leases.c.address == address)
            )
            ...
            sel_cols = [
                obj.leases,
                (
                    (sqlalchemy.sql.func.now() - obj.leases.c.starts)
                    < text("interval '%s sec'" % min_lease_age)
                ).label("recent"),
                (text("extract( epoch from leases.ends - NOW() )::int AS time_left")),
            ]
            query = select(*sel_cols).where(lease_cond).with_for_update()
            result = self._execute(query)
            ...
            if result:
                if result[0].recent or result[0].time_left > expires:
                    ...
                    if self.debug and result[0].recent:
                        print(
                            "Recent match (< %s s old) found: %s"
                            % (min_lease_age, str(result))
                        )
                    else:
                        print(
                            "Longer existing lease found (requested %s): %s"
                            % (expires, str(result))
                        )
                    self._commit()
                    return result
                ...
                if self.debug:
                    a = self._execute(
                        select(obj.leases).where(obj.leases.c.address == address)
                    )
        ...
        query = select(obj.leases).where(
            and_(obj.leases.c.mac == mac, obj.leases.c.address == address)
        )
        result = self._execute(query)
        if not result:
            raise Exception(
                f"Could not create lease for mac: {mac} address: {address}"
            )
        else:
            if self.debug:
                print(f"mac: {mac} address: {address} matching lease: {result}")
    ...
    def get_valid_nets(self, gateway):
        ...
        # It is cleaner to define the join clause first
        network_join = obj.networks.join(
            net_alias,
            and_(
                net_alias.c.network.op(">>")(gateway),
                obj.networks.c.shared_network == net_alias.c.shared_network,
            ),
        )
        ...
        if not networks:
            for i in self._execute(
                select(
                    obj.networks.c.network
                ).where(obj.networks.c.network.op(">>")(gateway))
            ):
                networks.append(i[0])
            if not networks:
    ...
    def valid_addresses_q(self, networks, registered):
        ...
        valid_addrs = obj.addresses.join(
            obj.networks,
            and_(
                obj.networks.c.network == obj.addresses.c.network,
                obj.networks.c.network.in_(networks),
            ),
        )
        columns = [
            obj.addresses.c.address,
            ...
            obj.networks.c.network,
            obj.networks.c.gateway,
            obj.pools.c.lease_time,
        ]
        if registered:
            addrs = valid_addrs.outerjoin(
                obj.pools, obj.pools.c.id == obj.addresses.c.pool
            )
        else:
            addrs = valid_addrs.join(
                obj.pools, and_(obj.pools.c.id == obj.addresses.c.pool)
            )
        addrs = addrs.outerjoin(
            obj.leases, obj.leases.c.address == obj.addresses.c.address
        )
    ...
    def _make_dhcp_lease(
        ...
        if self.debug:
            print(f"valid networks for {gateway}: {str(networks)}")
        if not self.lock_mac(mac):
            raise DHCPRetryError(mac)
        ...
        if not unregistered and not disabled:
            is_static = False
            allowed_pools_q = select(obj.hosts_to_pools.c.pool_id).where(
                obj.hosts_to_pools.c.mac == mac
            )
            ...
            columns, registered_addrs = self.valid_addresses_q(
                networks, registered=True
            )

            registered_q = (
                select(*columns).select_from(registered_addrs)
                .where(
                    or_(
                        and_(
                            or_(
            ...
            if not discover and address and requested_address != address.address:
                ...
                raise error.InvalidIPAddress(
                    "Requested address %s for host %s from gateway %s not allowed"
                    % (requested_address, mac, gateway)
                )

            if address and not discover:
                if (
                    self.lock_address(address.address)
                    and not is_static
                    and not discover
                ):
                    ...
                    q = obj.dhcp_dns_records.delete().where(
                        or_(
                            and_(
                                ...
                                obj.dhcp_dns_records.c.name != hostname,
                            ),
                            and_(
                                obj.dhcp_dns_records.c.ip_content != address.address,
                                obj.dhcp_dns_records.c.name == hostname,
                    ...
                    q = select(obj.dhcp_dns_records).where(
                        and_(
                            obj.dhcp_dns_records.c.ip_content == address.address,
                            obj.dhcp_dns_records.c.name == hostname,
                        )
                    ...
                    if not exists:
                        ...
                        self.add_dhcp_dns_record(
                            name=hostname,
                            ip_content=address.address,
                            ttl=dynamic_address_ttl,
                        )

        if unregistered or disabled or (is_static and not address):
            if address:
                raise Exception(
                    "FIXME: unregistered or disabled host got an address: %s" % address
                )
            if self.debug:
            ...
            # check the requested address and see if it 'works'
            unregistered_q = select(*columns).select_from(unreg_addrs).where(
                or_(
            ...
            if not address:
                leased_q = (
                    select(*columns).select_from(unreg_addrs)
                    .where(obj.leases.c.mac == mac)
                ...
                if leased:
                    if self.debug:
                        print(
                            "Found existing dynamic lease for this unregistered host."
                        )
                    address = leased[0]
            ...
            if not address:
                ...
                addresses = self._execute(addresses_q)
                address = search_addresses(
                    addresses, "Using expired unregistered lease. %s %s"
                )
        ...
        if not address:
            ...
            raise error.NotFound(
                f"No valid leases found for client {mac} from gateway {gateway}"
            )
        elif make_lease:
            ...
            lease_time_option = self.retrieve_dhcp_options(
                mac, address.address, [LEASE_TIME_OPTION]
            )
            if lease_time_option:
                ...
                if lease_time > MAX_LEASE_TIME:
                    raise Exception(
                        "Bad lease time: %s (%s)"
                        % (lease_time, dict(lease_time_option))
                    )

                    # FIXME: we should check lease_time here, but oh well
            self.update_or_create_lease_and_expire_conflicting(
    ...
    def retrieve_dhcp_options(self, mac, address, option_ids):
        ...
        host_grp = self._execute(
            select(obj.hosts.c.dhcp_group).where(obj.hosts.c.mac == mac)
        )
        if host_grp:
            host_grp = host_grp[0][0]
        else:
            host_grp = None

        pool_grp = self._execute(
            select(
                obj.pools.c.dhcp_group
            ).select_from(
                obj.pools.join(
                    obj.addresses,
                    and_(
                        obj.addresses.c.address == address,
                        obj.addresses.c.pool == obj.pools.c.id,
                    ),
                ),
            )
        )
        if pool_grp:
            pool_grp = pool_grp[0][0]
        else:
            pool_grp = None
        ...
        shared_net_grp = None
        if shared_net_grp:
            shared_net_grp = shared_net_grp[0][0]
        else:
            shared_net_grp = None

        network_grp = self._execute(
            select(obj.networks.c.dhcp_group).where(
                obj.networks.c.network.op(">>")(address)
            )
        )
        if network_grp:
            network_grp = network_grp[0][0]
        else:
            network_grp = None
        ...
        options = select(
            obj.dhcp_options_to_dhcp_groups.c.oid,
            obj.dhcp_options_to_dhcp_groups.c.value,
        ).where(obj.dhcp_options_to_dhcp_groups.c.gid.in_(grp_lst))
        if option_ids:
            options = options.where(
                obj.dhcp_options_to_dhcp_groups.c.oid.in_(option_ids)
            )
        ...
        if grp_order_mapping:
            options = options.order_by(
                sqlalchemy.sql.case(
                    *grp_order_mapping, value=obj.dhcp_options_to_dhcp_groups.c.gid
                )
            )
        else:
            raise Exception(
                "FIXME: no DHCP groups found that apply to this mac/address (%s/%s)"
                % (mac, address)
            )
