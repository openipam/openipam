...
dhcp_options_to_dhcp_groups = Table(
    "dhcp_options_to_dhcp_groups", meta, autoload_with=engine
)
...
structured_attribute_values = Table(
    "structured_attribute_values", meta, autoload_with=engine
)
...
if backend.enable_gul:
    gul_recent_arp_byaddress = Table(
        "gul_recent_arp_byaddress", meta, autoload_with=engine
    )
