...
class MainWebService(XMLRPCController):
    ...
    def __getattribute__(self, name):
        ...
        if (
            ...
            and name[0] != "_"
            and name not in ["default", "log_call", "login"]
        ):
            ...
            try:
                if "user" in cherrypy.session:
                    ...
                    db = interface.DBInterface(
                        uid=cherrypy.session["user"]["uid"],
                        username=cherrypy.session["user"]["username"],
                        min_perms=cherrypy.session["user"]["min_permissions"],
                    )
                    return db
            finally:
                cherrypy.session.release_lock()
    ...
    def login(self, username, password):
        ...
        try:
            ...
            cherrypy.log(
                "Successful login: %s" % str(user.__dict__),
                context="",
            ...
            cherrypy.log(
                "Failed Login: User does not have Email address: %s" % username,
                context="",
                severity=logging.DEBUG,
                traceback=False,
            )
            ...
            cherrypy.log(
                "Failed Login (type %s): %s %s" % (type(e), username, e.message),
                context="",
                severity=logging.DEBUG,
            ...
            raise error.InvalidCredentials(
                "Invalid credentials; username: %s" % username
            )
        finally:
            cherrypy.session.release_lock()
    ...
    def has_min_perms(self, perms):
        cherrypy.session.acquire_lock()
        try:
            return Perms(cherrypy.session["user"]["min_permissions"]) & Perms(
                perms
            ) == Perms(perms)
        finally:
            cherrypy.session.release_lock()
    ...
    def find_ownernames_of_host(self, *args):
        ...
        for group in groups:
            data = dict(group)
            if (
                self.has_min_perms(backend.show_user_perms)
    ...
    def validate_host_info(self, *args):
        ...
        if (
            ...
            or (
                not kw["editing"]
                and "owners" not in kw
                and (
                    "add_host_to_my_group" not in kw
                    or ("add_host_to_my_group" in kw and not kw["add_host_to_my_group"])
                )
            )
            ...
            messages.append(
                "Some information required to perform the action was not supplied. %s"
                % str(kw)
            )

        if kw["editing"] and "old_mac" not in kw:
            raise error.RequiredArgument(
                "When editing, old_mac is a required argument."
            )
        if (
            not kw["is_dynamic"]
            and not kw["editing"]
            and (
                ("address" not in kw or not kw["address"])
                and ("network" not in kw or not kw["network"])
            )
        ):
            messages.append(
                "This is a static IP registration and the network was not specified."
            )
        ...
        if "domain" in kw and kw["domain"] and type(kw["domain"]) is not int:
            raise error.InvalidArgument(
                "The domain specified must be an integer and must be a domain ID."
            )
        if (
            "expiration" in kw
            and kw["expiration"]
            and type(kw["expiration"]) is not int
        ):
            raise error.InvalidArgument(
                "The expiration specified must be an integer and must be an expiration ID."
            )
        if (
            "owners" in kw
            and kw["owners"]
            and type(kw["owners"]) is not list
            and type(kw["owners"]) is not tuple
        ):
            raise error.InvalidArgument(
                "The owners argument specified must be a list or tuple of group names."
            )
        ...
        if (
            kw["mac"]
            and not validation.is_mac(kw["mac"])
            and kw["mac"] not in ["vmware"]
        ):
            messages.append("The specified MAC address is invalid.")
        if (not kw["editing"] and not kw["hostname"]) or (
            kw["hostname"] and not validation.is_hostname(kw["hostname"])
        ):
            messages.append(
                "The specified hostname is invalid. Please use only letters, numbers, and dashes."
            )
        if (
            not kw["is_dynamic"]
            and "network" in kw
            and kw["network"]
            and not validation.is_cidr(kw["network"])
        ):
            messages.append(
                "The specified network is invalid. Please give a valid network in CIDR notation."
            )
        ...
        if not domain:
            messages.append(
                "Insufficient permissions to add host to domain %s" % kw["domain"]
            )
        ...
        if kw["editing"]:
            old_host = self.get_hosts(
                {"mac": kw["old_mac"], "additional_perms": perms.OWNER}
            )
            if not old_host:
        ...
        if not backend.allow_non_admin_host_transfer and (
            "add_host_to_my_group" not in kw
            or ("add_host_to_my_group" in kw and not kw["add_host_to_my_group"])
        ):
            ...
            if not has_min_admin_perms:
                ...
                users_groups = self.get_groups(
                    {"uid": self.get_uid(), "additional_perms": perms.OWNER}
                )
        ...
        if not kw["editing"]:
            if validation.is_mac(kw["mac"]):
                ...
                host_by_mac = []
            host_by_name = self.get_hosts(
                {"hostname": kw["hostname"], "show_expired": False}
            )
        else:
            ...
            if (
                "hostname" in kw
                and kw["hostname"]
                and (kw["hostname"] != old_host["hostname"])
            ):
        ...
        if "expiration" in kw and kw["expiration"]:
            ...
            expiration = (
                misc.make_time_delta(expiration)
    ...
    def edit_host(self, *args):
        ...
        if "old_mac" not in args[0]:
            raise error.RequiredArgument(
                "Must specify the old MAC address for edit_host"
            )
    ...
    def get_hosts(self, *args):
        ...
        if (
            "mac" in args[0]
            and args[0]["mac"]
            and not validation.is_mac(args[0]["mac"])
        ):
            raise error.InvalidMACAddress()
        if (
            "endmac" in args[0]
            and args[0]["endmac"]
            and not validation.is_mac(args[0]["endmac"])
        ):
            raise error.InvalidMACAddress()
    ...
    def get_next_hostname(self, *args):
        ...
        if "domain" not in args[0]:
            raise error.RequiredArgument(
                "Must specify both domain for getting the next hostname."
            )
    ...
    def validate_dns_records(self, *args):
        ...
        def validate_syntax(row, new_record=False):
            ...
            if "deleted" in row:
                if "id" not in row:
                    raise error.RequiredArgument(
                        "Deleted rows need to specify 'id' key. Row info: %s" % row
                    )
                else:
                    return
            ...
            if (
                new_record
                and not row["tid"] == 1
                and ("name" not in row or "text_content" not in row)
            ):
                raise error.RequiredArgument(
                    "Name and content are required for new records."
                )
            ...
            if "name" in row:
                ...
                if not validation.is_fqdn(row["name"]) and row["tid"] not in [16]:
                    temp_messages.append(
                        "Invalid name, use a fully qualified domain name: %s ( %s )"
                        % (row["name"], row["text_content"])
                    )
            ...
            if "text_content" in row:
                ...
                if row["tid"] in (2, 5, 12, 15):
                    if not validation.is_fqdn(row["text_content"]):
                        temp_messages.append(
                            "Invalid fully qualified domain name: %s"
                            % row["text_content"]
                        )
                        # SOA
                elif row["tid"] == 6:
                    if not validation.is_soa_content(row["text_content"]):
                        temp_messages.append(
                            "Invalid SOA content: %s" % row["text_content"]
                        )
                        # SRV
                elif row["tid"] == 33:
                    if not validation.is_srv_content(row["text_content"]):
                        temp_messages.append(
                            "Invalid SRV content: %s" % row["text_content"]
                        )
                elif row["tid"] == 44:
                    if not re.match("^[12] 1 [0-9A-F]{40}", row["text_content"]):
                        temp_messages.append(
                            "Invalid SSHFP content: %s" % row["text_content"]
                        )
                elif row["tid"] == 16:
                    pass  # freeform field
                else:
                    raise error.NotImplemented(
                        "Validation for tid %s has not been implemented yet."
                        % row["tid"]
                    )
            ...
            if "ttl" in row:
                raise error.NotImplemented(
                    "Validation for changes in TTL values is not currently supported."
                )
        ...
        def set_priority(row):
            ...
            if row["tid"] != 1:
                if row["tid"] == 15:  # MX
                    match = re.compile("^([0-9]{1,2}) (.*)$").search(
                        row["text_content"]
                    )
                elif row["tid"] == 33:  # SRV
                    match = re.compile("^([0-9]{1,2}) (\d+ \d+ .*)$").search(
                        row["text_content"]
                    )
        ...
        def validate_arecord(ip_content):
            ...
            if not address:
                messages.append(
                    "A Record must be in an allocated network. Could not add record: %s %s"
                    % (form_row["name"], form_row["ip_content"])
                )

            if address and address[0]["mac"] == None:
                messages.append(
                    "A Record must point to a static IP. Could not add record: %s %s"
                    % (form_row["name"], form_row["ip_content"])
                )
        ...
        fqdn_perms = db.find_domain_permissions_for_fqdns(
            [row["name"] for row in new_records]
        )
        fqdn_perms = fqdn_perms[0] if fqdn_perms else {}

        dns_type_perms = self.get_dns_types(
            {"only_useable": True, "make_dictionary": True}
        )
        ...
        for form_row in form_submission:  # loop over results from form
            ...
            if "id" not in form_row:
                ...
                if form_row["name"] in fqdn_perms:
                    ...
                    print(
                        "FIXME: !fqdn_perms.has_key(%s): value: %s"
                        % (form_row["name"], str(fqdn_perms))
                    )
                ...
                if not (
                    self.has_min_perms(rtype_perms)
                    or self.check_perms(rtype_perms, domain_perms)
                ):
                    messages.append(
                        "Insufficient permissions to add record %s" % form_row["name"]
                    )
                ...
                if not (Perms(id_perms[form_row["id"]]) & perms.DELETE == perms.DELETE):
                    messages.append(
                        "Insufficient permissions to delete record %s"
                        % form_row["name"]
                    )
                else:
            ...
            for backend_row in result:
                if backend_row["id"] == form_row["id"]:
                    ...
                    if did_change:
                        ...
                        if not (
                            Perms(id_perms[form_row["id"]]) & perms.MODIFY
                            == perms.MODIFY
                        ):
                            messages.append(
                                "Insufficient permissions to modify record %s"
                                % form_row["name"]
                            )
    ...
    def __generate_ticket_name(self):
        ...
        structure.append(
            "%s%s%s%s%s%s" % (group[0], vowel[0], cons[0], cons[1], vowel[1], group[1])
        )
    ...
    def arp_data(self, kw):
        ...
        if mac and ip or (not mac and not ip):
            raise error.InvalidArgument(
                "Specify exactly one of mac(%s) and ip(%s)." % (mac, ip)
            )
        ...
        if not self.has_min_perms(arp.min_permissions):
            raise error.InsufficientPermissions(
                "You are not permitted (have: %s, need: %s)"
                % (self.get_min_perms(), arp.min_permissions)
            )
        if mac:
            data = arp.bymac(mac)
        else:
    ...
    def create_shared_network(self, *args):
        ...
        if not args:
            raise error.RequiredArgument(
                "create_shared_network must get a dict of args."
            )
        ...
        db._begin_transaction()
        try:
            result = db.add_shared_network(
                name=args[0]["name"], description=args[0]["description"]
            )
    ...
    def register_guest(self, *args):
        ...
        if "ip" not in args[0] or "ticket" not in args[0]:
            raise error.RequiredArgument(
                "Must pass both ip and ticket to register_guest"
            )
        ...
        if auth.guests_multi_use_tickets:
            ...
            macaddr = self.__sanitize(
                __guest_db.find_mac_from_lease(**{"ip": args[0]["ip"]})
            )
            ...
            if not macaddr:
                ...
                raise error.NotFound(
                    "The MAC address for this guest's host could not be found. Ticket: %s IP:%s"
                    % (args[0]["ticket"], args[0]["ip"])
                )
            ...
            hosts = __guest_db.get_hosts(
                hostname=hostname_fmt % "%%", limit=1, funky_ordering=True
            )
            ...
            if hosts:
                ...
                try:
                    ...
                    hostname = int(
                        hosts[0]["hostname"][
                            hostname_fmt.find("%s") : hosts[0]["hostname"].find(
                                end_part
                            )
                        ]
                    )
                    ...
                    raise error.FatalException(
                        "Couldn't determine format for guest hostname ... got last host: %s"
                        % hosts
                    )
            else:
            ...
            try:
                ...
                __guest_db.add_host_to_group(
                    mac=macaddr, gid=backend.db_default_guest_group_id
                )
