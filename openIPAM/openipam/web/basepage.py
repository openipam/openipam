...
class BasePage(object):
    ...
    def __init__(self):
        ...
        if frontend.xmlrpc_ssl_enabled:
            ...
            self.__url = "https://%s:%s/api/" % (
                frontend.xmlrpc_host,
                frontend.xmlrpc_port,
            )
        else:
            # Remember the trailing slash! /
            self.__url = "http://%s:%s/api/" % (
                frontend.xmlrpc_host,
                frontend.xmlrpc_port,
            )
        ...
        try:
            ...
            if hasattr(cherrypy.session, "username"):
                raise error.FatalException(
                    "Still have session username ... this shouldn't ever happen ... please tell the openIPAM developers about this error and the conditions under which it happened."
                )
        finally:
    ...
    def check_session(self, logging_in=False):
        ...
        try:
            if "transport" not in cherrypy.session:
                cherrypy.session["transport"] = CookieAuthXMLRPCSafeTransport(
                    ssl=frontend.xmlrpc_ssl_enabled
                )

            self.webservice = xmlrpc.client.ServerProxy(
                self.__url, transport=cherrypy.session["transport"], allow_none=True
            )
        ...
        **kw
        ...
        if not username and not password:
            ...
            try:
                ...
                cherrypy.session["has_global_owner"] = (
                    Perms(info["min_permissions"]) & perms.OWNER
                ) == perms.OWNER
