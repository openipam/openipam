...
class Basics(object):
    ...
    def __init__(self, section, javascript=""):
        ...
        self.__sections = {
            "hosts": '<a href="/hosts/">Manage Hosts</a>',
    ...
    def wrap(
        ...
        if filename and values:
            ...
            try:
                ...
                values["has_admin_perms"] = (
                    Perms(cherrypy.session["min_permissions"]) & perms.ADMIN
                ) == perms.ADMIN
                values["has_owner_perms"] = (
                    Perms(cherrypy.session["min_permissions"]) & perms.OWNER
                ) == perms.OWNER
                values["has_deity_perms"] = (
                    Perms(cherrypy.session["min_permissions"]) & perms.DEITY
                ) == perms.DEITY
        ...
        text.append(
			...
			%s
			<div id="main">"""
            % self.begin(extra_headers=extra_headers)
        )
    ...
    def footer(self):
        ...
        if type(self.__javascript) is tuple:
            for inc in self.__javascript:
                includes.append(
                    '<script type="text/javascript" src="%s"></script>' % inc
                )
        else:
            includes.append(
                '<script type="text/javascript" src="%s"></script>' % self.__javascript
            )
