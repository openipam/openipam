...
class Hosts(BasePage):
    ...
    def leftnav_actions(self, current=None):
        ...
        return submenu(
            values=actions, links=action_links, title="Actions", selected=current
        )
    ...
    def leftnav_options(self):
        ...
        options = (
            ("Show expired hosts", "Hide expired hosts"),
            ("Show everyone's hosts", "Show only my hosts"),
        )
        ...
        try:
            selected = (
                cherrypy.session["show_expired_hosts"],
                cherrypy.session["show_all_hosts"],
            )
        finally:
    ...
    def get_leftnav(self, action="", show_options=True):
        return "%s%s" % (
            self.leftnav_actions(action),
            (self.leftnav_options() if show_options else ""),
        )
    ...
    def get_hosts(
        ...
        if mac:
            if "*" in mac:
                ...
                if not re.match(r"[0-9a-fA-F]{6,11}", tmpmac):
                    raise Exception(
                        "Must give between 6 and 11 hex digits of valid MAC address for wildcarding: %s (%s)"
                        % (mac, tmpmac)
                    )
        ...
        for host in hosts:
            ...
            host["description"] = (
                host["description"].encode("utf8") if host["description"] else ""
            )
        ...
        for host in hosts:
            if host["mac"] in perms:
                host["has_permissions"] = (
                    Perms(perms[host["mac"]]) & frontend.perms.OWNER
                ) == frontend.perms.OWNER
            else:
    ...
    def mod_host_attributes(self, values=None):
        ...
        nets = self.webservice.get_networks(
            {"additional_perms": str(frontend.perms.ADD), "order_by": "network"}
        )
        nets_by_type = {}
        ...
        for v, k in sorted(
            [
                (self.address_types[i]["description"], i)
                for i in list(nets_by_type.keys())
            ]
        ):
        ...
        for k in nets_by_type_keys:
            ...
            values["nets_by_type"].append(
                (k, cjson.encode(netlist).replace("'", "&#39;"))
            )
            if len(netlist) > 0:
                values["have_networks"] = True
        values["domains"] = self.webservice.get_domains(
            {
                "additional_perms": str(frontend.perms.ADD),
                "show_reverse": False,
                "order_by": "name",
            }
        )
        values["expirations"] = self.webservice.get_expiration_types()
        values["groups"] = self.webservice.get_groups(
            {"ignore_usergroups": True, "order_by": "name"}
        )
        values["dhcp_groups"] = self.webservice.get_dhcp_groups({"order_by": "name"})

        values["address_types"] = [
            (self.address_types[k]["name"], self.address_types[k]["description"])
            for k in nets_by_type_keys
        ]
        ...
        for k in list(self.address_types.keys()):
            ...
            if "default" in t and t["default"]:
                if default is not None:
                    raise Exception(
                        "Bad configuration -- must only specify one default (%s and %s both marked default)"
                        % (default, k)
                    )
                default = k
        if not default:
    ...
    def add_host(self, **kw):
        ...
        is_dynamic = (
            "pool" in addr_type
            and addr_type["pool"] is not None
            and not addr_type["ranges"]
        )
        ...
        mac = self.webservice.register_host(
            {
                ...
                "network": (
                    kw["network"] if "network" in kw and kw["network"] else None
                ),
                "add_host_to_my_group": False,
                "address": (kw["ip"] if "ip" in kw else None),
                "dhcp_group": (
                    kw["dhcp_group"]
                    if "dhcp_group" in kw and kw["dhcp_group"]
                    else None
                ),
            }
        )
    ...
    def edit_host(self, **kw):
        ...
        changed_to_static = "did_change_ip" in kw or (
            "was_dynamic" in kw and not is_dynamic
        )
        ...
        self.webservice.change_registration(
            {
                ...
                "description": kw["description"],
                "expiration": (
                    int(kw["expiration"]) if "did_renew_host" in kw else None
                ),
        ...
        self.redirect(
            "/hosts/search/?q=%s"
            % misc.fix_mac(kw["mac"] if kw["mac"] else kw["old_mac"])
        )
    ...
    def index(self, page=0, **kw):
        ...
        try:
            ...
            if "show_expired" in kw:
                cherrypy.session["show_expired_hosts"] = not cherrypy.session[
                    "show_expired_hosts"
                ]
                redirect_to_referer()
            if "show_all" in kw:
                cherrypy.session["show_all_hosts"] = not cherrypy.session[
                    "show_all_hosts"
                ]
                redirect_to_referer()
            ...
            if cherrypy.session["has_global_owner"]:
                ...
                self.redirect(
                    "/hosts/search/?username=%s" % cherrypy.session["username"]
                )
        ...
        return self.__template.wrap(
            ...
            filename="%s/templates/hosts.tmpl" % frontend.static_dir,
            values=values,
        )
    ...
    def add(self, **kw):
        ...
        return self.__template.wrap(
            leftcontent=self.get_leftnav(action="Add Host", show_options=False),
            filename="%s/templates/mod_host.tmpl" % frontend.static_dir,
            values=values,
        )
    ...
    def edit(self, macaddr=None, **kw):
        ...
        host = self.webservice.get_hosts(
            {"mac": macaddr, "additional_perms": str(frontend.perms.MODIFY)}
        )
        if not host:
        ...
        domain = self.webservice.get_domains(
            {
                "contains": str(host["hostname"]),
                "additional_perms": str(frontend.perms.ADD),
            }
        )
        ...
        values["host"]["description"] = (
            values["host"]["description"].encode("utf8")
            if values["host"]["description"]
            else ""
        )
        ...
        return self.__template.wrap(
            leftcontent=self.get_leftnav(show_options=False),
            filename="%s/templates/mod_host.tmpl" % frontend.static_dir,
            values=values,
        )
    ...
    @cherrypy.expose
    def search(
        self, q="", expiring=False, page=0, order_by="hostname", success=False, **kw
    ):
        """
            ...
            if validation.is_mac(element):
                ...
                if stype in special_search:
                    ...
                    raise error.InvalidArgument(
                        "Unrecognized special search type: %s (value: %s)"
                        % (stype, value)
                    )
                if stype == "mac" and "*" not in value and len(value) >= 24:
                    ...
                    if not re.match(
                        r"([0-9a-fA-F]{6})[0-9a-fA-F]{6}\1[0-9a-fA-F]{6}", rawmacs
                    ):
                        raise error.InvalidArgument(
                            "Invalid mac range: %s (%s)" % (value, rawmacs)
                        )
                    kw["mac"] = rawmacs[:12]
                ...
                if "namesearch" in kw:
                    raise error.InvalidArgument(
                        "Invalid search string -- more than one name (%s, %s)"
    ...
    def host_info(self, mac, wrap=True):
        ...
        vals["dns_records"] = self.webservice.get_dns_records(
            {"mac": mac, "order_by": "tid,name,ip_content,text_content"}
        )
        ...
        if frontend.enable_gul:
            ...
            gul_byaddr = self.webservice.get_gul_recent_arp_byaddress(
                {"address": addrlist}
            )
        ...
        if wrap:
            return self.__template.wrap(
                leftcontent=self.get_leftnav(), filename=filename, values=vals
            )
    ...
    def add_attribute(
        ...
        if submit:
            if attrs[int(attr_type_id)]["structured"]:
                self.webservice.add_structured_attribute_to_host(
                    {"mac": mac, "avid": int(structured_value)}
                )
            else:
                self.webservice.add_freeform_attribute_to_host(
                    {"mac": mac, "aid": attr_type_id, "value": freeform_value}
                )
            status = "Success."
            if wrap:
    ...
    def del_attribute(self, mac=None, aid=None, structured=None, avid=None, value=None):
        ...
        if structured is not None and structured.lower() in [
            "false",
            "0",
            "f",
            "no",
        ...
        if structured:
            ...
            self.webservice.del_freeform_attribute_to_host(
                {"mac": mac, "aid": aid, "value": value}
            )
