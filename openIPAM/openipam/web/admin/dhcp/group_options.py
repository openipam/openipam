...
class AdminDHCPGroupOptions(AdminDHCPGroups):
    ...
    def get_options_table(self, gid):
        ...
        jquery = (
            """<script type="text/javascript">
						function confirmOptionRemove( rid ){
							$("#delOption" + rid)
								.after('<span id="confirm'+rid+'">Are you sure? <a href="" onclick="removeOption('+rid+', """
            + str(gid)
            + """); return false;">Yes</a> / <a href="" onclick="restoreOptionRemove('+rid+'); return false;">No</a></span>')
								.hide(); 
						};
						...
						function removeOption( rid ){
							$.ajax({
								type: "GET",
								url: "/ajax/ajax_del_dhcp_option_from_group/?oid="+rid+"&gid="""
            + str(gid)
            + """",
								success: function() {
										$("#option" + rid).remove();
        ...
        item_template = (
            """<tr class="info" id="option%(rid)s">
							<td>%(name)s</td>
							<td>%(option)s</td>
							<td>%(value)s</td>
							<td class="actions">
								<a href="/admin/dhcp/groups/options/edit/?rid=%(rid)s&gid="""
            + str(gid)
            + """">Edit</a> | 
								<a href="/admin/dhcp/groups/options/remove/?rid=%(rid)s&gid="""
            + str(gid)
            + """" id="delOption%(rid)s" onclick="confirmOptionRemove(%(rid)s); return false;">Remove</a>
							</td>
						</tr>
						"""
        )
    ...
    def option_form(self, gid, action_string="Add", rid=0):
        ...
        for anOption in options:
            ...
            form += (
                ...
                + str(anOption["id"])
                + '"'
                + ((option["oid"] == anOption["id"]) and ' selected=""' or "",)[0]
                + ">"
                + anOption["name"]
                + "</option>"
            )
        ...
        form += (
            '''</select>
				...
				<div id="element">
					...
					<div id="value"><input class="text" name="value" value="'''
            + option["value"]
            + '''" /></div>
				</div>
    ...
    def index(self, gid):
        ...
        leftnav = str(self.leftnav_manage("DHCP Groups")) + str(
            self.leftnav_option_actions(gid)
        )
        group = self.webservice.get_dhcp_groups({"gid": gid})[0]

        maincontent = (
            "<h1>" + group["name"] + " DHCP Options</h1>" + self.get_options_table(gid)
        )
    ...
    def add(self, gid=None):
        ...
        leftnav = str(self.leftnav_manage("DHCP Groups")) + str(
            self.leftnav_option_actions(gid, "Add DHCP Option to Group")
        )
    ...
    def process_option_form(self, oid, gid, value, rid=0):
        ...
        if rid:
            self.webservice.edit_dhcp_option_in_group(
                {"rid": rid, "oid": oid, "value": value}
            )
        else:
            self.webservice.add_dhcp_option_to_group(
                {"oid": oid, "gid": gid, "value": value}
            )
    ...
    def edit(self, rid, gid):
        ...
        maincontent = "<h1>Edit Option in Group</h1>" + self.option_form(
            gid, "Update", rid
        )
