...
class AdminGroups(Admin):
    ...
    def leftnav_view_actions(self, gid, current=None):
        ...
        action_links = (
            "/admin/groups/user/add/%s" % gid,
            "/admin/groups/domain/add/%s" % gid,
            "/admin/groups/network/add/%s" % gid,
            "/admin/groups/host/add/%s" % gid,
        )
    ...
    def group_form(self, action_string="Add", gid=0):
        ...
        form = (
            """<form action="/admin/groups/process_group_form" method="post" class="form">
				<div id="element">
    ...
    def index(self, **kw):
        ...
        if "showug" in kw:
            ...
            text.append(
                '<a href="/admin/groups/?showug=True">Show default user groups</a></div>'
            )
    ...
    def process_group_form(self, **kw):
        ...
        if int(kw["gid"]) == 0:
            ...
            self.webservice.add_group(
                {"name": kw["name"], "description": kw["description"]}
            )
        else:
            # We're updating a group
            self.webservice.edit_group(
                {"gid": kw["gid"], "name": kw["name"], "description": kw["description"]}
            )
    ...
    def view(self, gid):
        ...
        maincontent = (
            "<h1>"
            + group["name"]
            + '</h1><input type="hidden" id="gid" value="%s" />' % gid
        )
