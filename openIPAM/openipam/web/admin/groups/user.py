...
class AdminGroupsUser(AdminGroups):
    ...
    def get_results_table(self, search, gid):
        ...
        if len(search):
            ...
            used_users_temp = self.webservice.get_users(
                {"gid": gid, "username": "%%%s%%" % search}
            )
        ...
        if not users:
            ...
            for permission in permissions:
                item_template += (
                    "			<li><a href=\"javascript:;\" onclick=\"changePermissions(%%(id)s, '%(name)s', '%(id)s');\">%(name)s</a></li>"
    ...
    def add(self, gid, search="", uid=None):
        ...
        leftnav = str(self.leftnav_manage("Groups")) + str(
            self.leftnav_view_actions(gid, "Add User to Group")
        )
        ...
        text.append(
            """<h1>Add User to Group: <a href="/admin/groups/view/%(id)s">%(name)s</a></h1>
