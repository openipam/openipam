...
class AdminGroupsHost(AdminGroups):
	...
	def add(self, gid, search='', mac=None):
		...
		maincontent = '<h1>Add Host to Group: <a href="/admin/groups/view/'+gid+'">' + group['name'] + '</a></h1>' + \
						'''
					<form action="/admin/groups/host/add/'''+gid+'''" method="get" class="form">
						<div id="element">
							<div id="label">Find host:</div>
							<div id="value"><input type="text" id="search" class="text" name="search" value="'''+search+'''"/><input type="submit" class="button" value="Search"></div>
						</div>
					</form>
					'''
