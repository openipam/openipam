...
soa_content = '%s hostmaster@usu.edu %s %s 3600 604800 3600' % (nsname, pdns_start_time, default_ttl)
...
def send( line ):
	if line[-1:] != '\n':
		print_error("String did not have required endline as the last character, added.\n")
		line += '\n'
...
if __name__ == '__main__':
	...
	while True:
		...
		if abi == 2 and len(fields) == 7 and fields[0] == 'Q':
			...
			print_error("Got unrecognized input from PowerDNS (wrong number of fields): '%s'\n" % line, 2)
			fail()
			continue
