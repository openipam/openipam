...
class AdminDHCPGroups(Admin):
	...
	def group_form(self, action_string="Add", gid=0):
		...
		form = '''<form action="/admin/dhcp/groups/process_group_form" method="post" class="form">
				<div id="element">
					<div id="label">DHCP group name*:</div>
					<div id="value"><input type="text" class="text" name="name" value="''' + group['name'] + '''" /></div>
				</div>
				<div id="element">
					<div id="label">Description:</div>
					<div id="value"><textarea class="text" name="description" style="width: 350px;">''' + group['description'] + '''</textarea></div>
				</div>
	...
	def add(self, gid=None):
		...
		leftnav = str(self.leftnav_manage("DHCP Groups")) + str(self.leftnav_group_actions("Add Group"))
	...
	def process_group_form(self, **kw):
		...
		if int(kw['gid']) == 0:
			...
			try:
				self.webservice.edit_dhcp_group( { 'gid':kw['gid'], 'name':kw['name'], 'description':kw['description'] } )
			except:
