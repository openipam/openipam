...
class IPAMLogManager( cherrypy._GlobalLogManager ):
	def __init__(self, appid=None, logger_root='cherrypy', client_ip_header=None, syslog=False):
		self.client_ip_header = client_ip_header
		cherrypy._cplogging.LogManager.__init__(self, appid=appid, logger_root=logger_root)
		self.screen = True
	def access(self):
